...
function useWorkflowStore(initialItems) {
  ...
  const completeStep = useCallback(
    (itemId, idx) => {
      const item = itemsRef.current.find((i) => i.notionId === itemId);
      if (!item || item.progress !== idx) return;
      const notionId = item.notionId;
      const prev = itemsRef.current;

      const newBic = window.STEPS[idx + 1]?.bic;
      setItems((current) => current.map((it) =>
        it.notionId === itemId
          ? { ...it, progress: idx + 1, subPhaseIdx: 0, blockerStep: null, blockerText: "", ...(newBic ? { bic: newBic } : {}) }
          : it
      ));
  ...
  const undoStep = useCallback(
    (itemId, idx) => {
      const item = itemsRef.current.find((i) => i.notionId === itemId);
      if (!item || item.progress !== idx + 1) return;
      const notionId = item.notionId;
      const prev = itemsRef.current;

      const revertBic = window.STEPS[idx]?.bic;
      setItems((current) => current.map((it) =>
        it.notionId === itemId
          ? { ...it, progress: idx, subPhaseIdx: 0, ...(revertBic ? { bic: revertBic } : {}) }
          : it
      ));
  ...
  const unblockStep = useCallback(
    (itemId) => {
      ...
      if (!item) return;
      const notionId = item.notionId;
      const stepIdx  = item.blockerStep;
      const prev = itemsRef.current;
  ...
  const advanceSubPhase = useCallback(
    (itemId, stepIdx, subPhaseIdx) => {
      const item = itemsRef.current.find((i) => i.notionId === itemId);
      if (!item || item.progress !== stepIdx) return;
      const notionId = item.notionId;
      const prev = itemsRef.current;
      const subPhases = window.STEPS[stepIdx]?.dmPhases || [];
      const isLast = subPhaseIdx >= subPhases.length - 1;
      const newBic = isLast ? window.STEPS[stepIdx + 1]?.bic : null;

      setItems((current) => current.map((it) => {
        if (it.notionId !== itemId) return it;
        if (isLast) {
          return { ...it, progress: stepIdx + 1, subPhaseIdx: 0, blockerStep: null, blockerText: "", ...(newBic ? { bic: newBic } : {}) };
        }
