...
async function ensureNotionSchema() {
  try {
    ...
    // "Estimated Progress" must be a plain number field to be writable via API.
    // Formula properties are read-only — if it's a formula the user must change it manually in Notion.
    const epProp = db.properties["Estimated Progress"];
    if (!epProp) {
      schemaUpdates["Estimated Progress"] = { number: { format: "number" } };
    } else if (epProp.type !== "number") {
      console.warn(
        `⚠️  "Estimated Progress" is type "${epProp.type}" — progress writes will be ignored by Notion.` +
        ` Change it to a plain Number property in your Notion database to enable syncing.`
      );
    }
...
function notionPageToItem(page, projectsMap) {
  ...
  const notionStatus = p["Item Status"]?.status?.name || "Backlog";

  return {
    id, notionId: page.id, name, project, bic, priority,
    progress, subPhaseIdx, blockerStep, blockerText: blockerText || "",
    notionStatus, props,
  };
}
...
function stepNotionProps(stepIdx) {
  ...
  // Notion "Percent" format stores a decimal (0–1) and multiplies by 100 for display
  const pct = Math.min(stepIdx, TOTAL_STEPS) / TOTAL_STEPS;
  const props = {
...
app.patch("/api/items/:pageId/block", async (req, res) => {
  ...
  const validBlockers = ["Awaiting Supplier","Awaiting Instruction","Awaiting RFI Response","Awaiting Design Info","Awaiting Scope"];
...
app.patch("/api/items/:pageId/subphase", async (req, res) => {
  ...
  try {
    if (nextSubPhaseIdx >= subPhases.length) {
      ...
      const subPct = (stepIdx + nextSubPhaseIdx / subPhases.length) / TOTAL_STEPS;
      await notion.pages.update({
        page_id: pageId,
        properties: {
