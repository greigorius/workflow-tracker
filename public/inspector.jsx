    ...
    <div className="ins-overlay" onClick={onClose}>
      <aside className="ins-panel" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="ins-body">
          {/* Block / unblock */}
          {mode === "block" && status !== "complete" && (
            <section className="ins-section">
              <div className="ins-sec-title">
                <i className="ti ti-alert-triangle" aria-hidden="true" style={{ color: "#D85A30" }}></i>
                {item.blockerStep !== null ? "Update blocker" : "Block this step"}
              </div>
              <div className="ins-field">
                <label className="ins-lbl">Reason</label>
                <select
                  className="ins-input"
                  value={draftReason}
              ...
              <div className="ins-field">
                <label className="ins-lbl">Ball in court</label>
                <select
                  className="ins-input"
                  value={draftBIC}
                  onChange={(e) => handleUpdateBIC(e.target.value)}
                >
                  {window.BIC_OPTIONS.map((o) => (
              ...
              <div className="ins-actions">
                ...
                <button className="ins-btn ins-btn-warn" onClick={handleSubmitBlock}>
                  ...
                  {item.blockerStep !== null ? "Save blocker" : "Block step"}
                </button>
              </div>
            </section>
          )}
                ...
                return (
                  <li key={i} className={`ins-step ins-step-${cs}`}>
                    ...
                    <span className="ins-step-tag">
                      ...
                      {cs === "blocked" && <i className="ti ti-alert-triangle" aria-hidden="true"></i>}
                      {cs === "active" && "active"}
                    </span>
                  </li>
                );
