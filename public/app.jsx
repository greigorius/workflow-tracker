...
const STATUS_OPTIONS = [
  "Ongoing", "Backlog", "Set Up", "In Design", "Awaiting Approval",
  "By Others", "In Production", "On Site", "Close Out",
  "Blocked", "Complete", "Omitted",
];

function StatusDropdown({ statuses, onToggle, onClear }) {
  ...
  return (
    <div className="filt-status-dropdown" ref={ref}>
      <button
        className={`filt-input filt-select filt-status-trigger ${statuses.length > 0 ? "is-active" : ""}`}
        onClick={() => setOpen((v) => !v)}
      >
        <span>{label}</span>
        <i className="ti ti-chevron-down" style={{ fontSize: 10, marginLeft: 4, opacity: 0.5 }} aria-hidden="true"></i>
      </button>
      {open && (
        <div className="filt-status-menu">
          {STATUS_OPTIONS.map((s) => (
            <label key={s} className={`filt-status-item ${statuses.includes(s) ? "is-checked" : ""}`}>
              <input
                type="checkbox"
                checked={statuses.includes(s)}
                onChange={() => onToggle(s)}
              />
              <span className={`filt-status-dot filt-status-dot-${s.toLowerCase().replace(/\s+/g, "-")}`}></span>
              {s}
            </label>
          ))}
          {statuses.length > 0 && (
            <button className="filt-status-clear" onClick={() => { onClear(); setOpen(false); }}>
              Clear selection
            </button>
          )}
        </div>
...
function App() {
  ...
  // ── Data loading ────────────────────────────────────────────────────────
  const [loadState, setLoadState] = React.useState("loading"); // "loading" | "ok" | "error" | "refreshing"
  const [loadError, setLoadError] = React.useState(null);

  const store = window.useWorkflowStore([]);

  const loadItems = React.useCallback((isRefresh = false) => {
  ...
  // ── Filters ─────────────────────────────────────────────────────────────
  const FILTER_STORAGE_KEY = "workflow-default-filters";
  const FILTER_DEFAULTS = { q: "", project: "All projects", bic: "All BIC", statuses: [] };

  const [filters, setFilters] = React.useState(() => {
    try {
      const saved = localStorage.getItem(FILTER_STORAGE_KEY);
      return saved ? { ...FILTER_DEFAULTS, ...JSON.parse(saved), q: "" } : FILTER_DEFAULTS;
    } catch { return FILTER_DEFAULTS; }
  });

  // Track whether the current filters match the saved defaults (to colour the pin)
  ...
  const isDefaultSaved = React.useMemo(() => {
    if (!savedFilters) return false;
    return (
      filters.project === (savedFilters.project ?? FILTER_DEFAULTS.project) &&
      filters.bic === (savedFilters.bic ?? FILTER_DEFAULTS.bic) &&
      JSON.stringify([...(filters.statuses || [])].sort()) ===
        JSON.stringify([...(savedFilters.statuses || [])].sort())
    );
  }, [filters, savedFilters]);

  const saveDefaultFilters = React.useCallback(() => {
    try {
      const toSave = { project: filters.project, bic: filters.bic, statuses: filters.statuses };
      localStorage.setItem(FILTER_STORAGE_KEY, JSON.stringify(toSave));
      setSavedFilters(toSave);
      store.actions.pushToast("Default view saved", "ok");
    } catch { store.actions.pushToast("Could not save default", "error"); }
  }, [filters, store.actions]);

  const toggleStatus = React.useCallback((status) => {
    setFilters((f) => {
  ...
  const filteredItems = React.useMemo(() => {
    return store.items.filter((it) => {
      ...
      if (filters.statuses && filters.statuses.length > 0) {
        if (!filters.statuses.includes(it.notionStatus)) return false;
      }
      return true;
    });
  }, [store.items, filters]);
    ...
    <div className="app">
      <header className="topbar">
        ...
        <div className="topbar-right">
          ...
          <button
            className={`sync-pill ${anyPending || loadState === "refreshing" ? "is-syncing" : ""}`}
            onClick={() => !anyPending && loadState !== "refreshing" && loadItems(true)}
            title="Refresh from Notion"
            style={{ cursor: anyPending || loadState === "refreshing" ? "default" : "pointer" }}
          >
            <span className="sync-dot"></span>
            <span className="sync-lbl">
              {loadState === "refreshing" ? "Refreshing…" : anyPending ? "Syncing…" : `Notion · ${store.items.length} items`}
            </span>
            {!anyPending && loadState !== "refreshing" && (
              <i className="ti ti-refresh" style={{ fontSize: 10, opacity: 0.5, marginLeft: 2 }} aria-hidden="true"></i>
            )}
          </button>
        </div>
      </header>
      ...
      {view !== "reports" && (
        <div className="filterbar">
          ...
          <StatusDropdown statuses={filters.statuses} onToggle={toggleStatus} onClear={() => setFilters((f) => ({ ...f, statuses: [] }))} />
          <div className="filt-spacer"></div>
          <div className="filt-count">{filteredItems.length} items</div>
          <button
            className={`filt-save-btn ${isDefaultSaved ? "is-saved" : ""}`}
            onClick={saveDefaultFilters}
            title={isDefaultSaved ? "Current filters are saved as default" : "Save current filters as default view"}
          >
